...
from trl.trainer.rloo_trainer import (Any, AutoModelForSequenceClassification, AutoProcessor, AutoTokenizer, BaseTrainer, DataLoader, Dataset, FSDP, GenerationConfig, IterableDataset, Path, PeftConfig, PeftModel, PreTrainedModel, PreTrainedTokenizerBase, ProcessorMixin, RLOOConfig, RLOOTrainer, RepeatSampler, RewardFunc, Sampler, SyncRefModelCallback, TrainerCallback, VLLMClient, Version, apply_chat_template, asyncio, atexit, bnb, broadcast_object_list, copy, create_model_from_path, datasets, defaultdict, deque, disable_dropout_in_model, disable_gradient_checkpointing, ensure_master_addr_port, entropy_from_logits, gather, gather_object, get_config_model_id, get_peft_model, identity, inspect, is_bitsandbytes_available, is_conversational, is_datasets_available, is_peft_available, is_peft_model, is_rich_available, is_vllm_available, logger, nanmax, nanmin, nanstd, nn, nullcontext, os, pad, partial, pd, prepare_deepspeed, prepare_fsdp, prepare_multimodal_messages, prepare_multimodal_messages_vllm, print_prompt_completions_sample, profiling_context, profiling_decorator, seed_worker, selective_log_softmax, set_seed, shuffle_sequence_dict, shutdown_event_loop_in_daemon, split_pixel_values_by_grid, split_tensor_dict, start_event_loop_in_daemon, textwrap, time, torch, unsplit_pixel_values_by_grid, unwrap_model_for_generation, use_adapter, wandb, AutoModelForSequenceClassification, AutoProcessor, AutoTokenizer, Dataset, GenerationConfig, IterableDataset, PeftConfig, PeftModel, PreTrainedModel, PreTrainedTokenizerBase, ProcessorMixin, RLOOConfig, RLOOTrainer, RewardFunc, SyncRefModelCallback, TrainerCallback, VLLMClient, asyncio, atexit, bnb, copy, create_model_from_path, datasets, defaultdict, deque, disable_dropout_in_model, ensure_master_addr_port, get_config_model_id, get_peft_model, identity, inspect, is_bitsandbytes_available, is_peft_available, is_peft_model, is_vllm_available, logger, nn, os, pad, pd, prepare_deepspeed, prepare_fsdp, set_seed, shutdown_event_loop_in_daemon, start_event_loop_in_daemon, time, torch, FSDP, Version, apply_chat_template, broadcast_object_list, copy, gather, gather_object, is_conversational, nullcontext, os, pad, pd, prepare_multimodal_messages, prepare_multimodal_messages_vllm, profiling_context, time, torch, unwrap_model_for_generation, FSDP, gather, is_peft_model, nn, nullcontext, os, pd, profiling_decorator, Any, profiling_decorator, shuffle_sequence_dict, split_pixel_values_by_grid, split_tensor_dict, torch, unsplit_pixel_values_by_grid, PeftModel, PreTrainedModel, is_peft_available, logger, os, torch, FSDP, nn, os, pd, FSDP, is_peft_model, nn, pd, torch)
                ...
                with profiling_context(self, "vLLM.generate"):
                    if is_conversational({"prompt": prompts[0]}):
                        all_outputs = self.llm.chat(
                            ...
                            use_tqdm=False,
                            chat_template_kwargs=self.chat_template_kwargs, lora_request = self.model.load_lora('rloo_trainer_lora_model', load_tensors = True))
                    else:
