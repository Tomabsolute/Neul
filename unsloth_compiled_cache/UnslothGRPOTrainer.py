...
from trl.trainer.grpo_trainer import (Any, AutoModelForSequenceClassification, AutoProcessor, AutoTokenizer, BaseTrainer, Callable, DataLoader, Dataset, FSDP, GRPOConfig, GRPOTrainer, GenerationConfig, IterableDataset, Path, PeftConfig, PeftModel, PreTrainedModel, PreTrainedTokenizerBase, ProcessorMixin, RepeatSampler, RewardFunc, RolloutFunc, Sampler, SyncRefModelCallback, TrainerCallback, VLLMClient, Version, _ForwardRedirection, add_response_schema, apply_chat_template, asyncio, atexit, bnb, broadcast_object_list, copy, create_model_from_path, datasets, defaultdict, deque, disable_dropout_in_model, disable_gradient_checkpointing, ensure_master_addr_port, gather, gather_object, get_config_model_id, get_training_chat_template, identity, inspect, is_bitsandbytes_available, is_conversational, is_datasets_available, is_jmespath_available, is_liger_kernel_available, is_peft_available, is_peft_model, is_rich_available, is_vllm_available, json, logger, nanmax, nanmin, nanstd, nn, nullcontext, os, pad, parse_response, partial, pd, prepare_deepspeed, prepare_fsdp, prepare_multimodal_messages, prepare_multimodal_messages_vllm, print_prompt_completions_sample, profiling_context, profiling_decorator, seed_worker, selective_log_softmax, set_seed, shuffle_sequence_dict, shutdown_event_loop_in_daemon, split_pixel_values_by_grid, split_tensor_dict, start_event_loop_in_daemon, sys, textwrap, time, torch, transformers, unsplit_pixel_values_by_grid, unwrap_model_for_generation, use_adapter, wandb, warnings, AutoModelForSequenceClassification, AutoProcessor, AutoTokenizer, Callable, Dataset, GRPOConfig, GRPOTrainer, GenerationConfig, IterableDataset, PeftConfig, PeftModel, PreTrainedModel, PreTrainedTokenizerBase, ProcessorMixin, RewardFunc, RolloutFunc, SyncRefModelCallback, TrainerCallback, VLLMClient, Version, add_response_schema, asyncio, atexit, bnb, copy, create_model_from_path, datasets, defaultdict, deque, disable_dropout_in_model, ensure_master_addr_port, get_config_model_id, get_training_chat_template, identity, inspect, is_bitsandbytes_available, is_jmespath_available, is_liger_kernel_available, is_peft_available, is_peft_model, is_vllm_available, logger, nn, os, pad, pd, prepare_deepspeed, prepare_fsdp, set_seed, shutdown_event_loop_in_daemon, start_event_loop_in_daemon, sys, time, torch, transformers, warnings, Any, apply_chat_template, copy, disable_gradient_checkpointing, gather, gather_object, nanmax, nanmin, nanstd, os, pad, pd, prepare_multimodal_messages, torch, use_adapter, FSDP, Version, apply_chat_template, broadcast_object_list, copy, gather, gather_object, is_conversational, json, nullcontext, os, pad, pd, prepare_multimodal_messages, prepare_multimodal_messages_vllm, profiling_context, time, torch, transformers, unwrap_model_for_generation, os, pad, selective_log_softmax, torch, transformers, Any, profiling_decorator, shuffle_sequence_dict, split_pixel_values_by_grid, split_tensor_dict, torch, unsplit_pixel_values_by_grid, PeftModel, PreTrainedModel, is_peft_available, logger, os, torch, FSDP, nn, os, pd, FSDP, is_peft_model, nn, pd, torch, GRPOTrainer, gather, nanmax, nanmin, os, pad, torch)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            ...
            vllm_guided_decoding_regex = vllm_guided_decoding_regex,**kwargs)
                    ...
                    with profiling_context(self, "vLLM.generate"):
                        if is_conversational({"prompt": prompts[0]}):
                            all_outputs = self.llm.chat(
                                ...
                                chat_template_kwargs=self.chat_template_kwargs,
                                tools=self.tools,
                                chat_template=self.chat_template, lora_request = self.model.load_lora('grpo_trainer_lora_model', load_tensors = True))
                        else:
                            all_outputs = self.llm.generate(
                                all_prompts, sampling_params=sampling_params, use_tqdm=False, lora_request = self.model.load_lora('grpo_trainer_lora_model', load_tensors = True))
    ...
    def __init__(
        ...
        **kwargs
        ...
        super().__init__(
            ...
            tools = tools,
            rollout_func = rollout_func,**kwargs)
