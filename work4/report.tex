...
服务器上可以使用：
\begin{verbatim}
LM_EPOCHS=150 EMB_EPOCHS=40 BATCH_SIZE=64 \
  bash work4/run_all_experiments.sh
\end{verbatim}

\section{实验结果展示}
服务器实验使用 CUDA 训练，LSTM 训练 150 轮，Skip-gram 训练 40 轮。训练完成后，LSTM 语言模型输出 \texttt{metrics.json}、\texttt{history.csv}、\texttt{generation\_samples.md} 和训练曲线；嵌入模型输出 \texttt{neighbors.md}、\texttt{neighbors.json} 与近邻可视化图。

\begin{table}[H]
\centering
\small
\caption{LSTM 语言模型服务器训练结果}
\begin{tabular}{c|c}
\toprule
项目 & 数值 \\
\midrule
...
\subsection{生成结果分析}
训练曲线显示，监督式题答训练后验证集损失在第 147 轮达到最低，最终保存验证损失最低的 \texttt{best\_lstm\_lm.pt}。在手动测试题「今有田廣十五步，從十六步」上，模型生成「一畝」，与原书答案一致，说明模型能复现部分典型题型。
...
股四尺，弦五尺，求句 & 三尺 & 三尺 \\
井徑五尺，不知其深 & 五丈七尺五寸 & 五丈二尺 \\
\bottomrule
\end{tabular}
\end{table}

从样例看，模型已经学习到《九章算术》的答案格式、数字表达和单位体系，例如「畝」「尺」「寸」「分之」等模式会稳定出现；部分短题和高频题型能够生成完全正确的答案，例如方田、约分和勾股样例。但模型仍不能可靠掌握精确算术推理，复杂均输、粮食换算和多变量问题中经常出现单位相关但数值错误的答案。关键词命中率为 0.6333，只能说明模型在部分验证样例中生成了相关数字或单位，不能等同于算术正确率。

\subsection{嵌入结果分析}
文本嵌入的近邻可以从另一个角度验证学习效果。服务器结果中，「粟」的近邻包含粗米、饭、稻、米等粮食换算相关字，「米」的近邻也集中在饭食和米制品语境中，说明模型较好捕捉到粟米章中的共现关系；「尺」的近邻包含「寸、丈、深、竿、绳」一类长度测量相关字，也符合勾股、井深类题目的语境。「田」的近邻中出现「頃」，「步」的近邻中出现「田」，说明面积单位关系也有一定体现。部分近邻如「惡、箕、圭」等不够理想，主要原因是字符级模型容易受到古文上下文和低频字共现干扰。
