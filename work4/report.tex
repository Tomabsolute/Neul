...
\section{实验结果展示}
服务器实验使用 CUDA 训练，LSTM 训练 80 轮，Skip-gram 训练 40 轮。训练完成后，LSTM 语言模型输出 \texttt{metrics.json}、\texttt{history.csv}、\texttt{generation\_samples.md} 和训练曲线；嵌入模型输出 \texttt{neighbors.md}、\texttt{neighbors.json} 与近邻可视化图。
...
\subsection{生成结果分析}
训练曲线显示，验证集损失在第 5 轮达到最低，此后训练损失继续下降而验证损失上升，说明小规模数据上出现了过拟合。因此实验保存并使用验证损失最低的 \texttt{best\_lstm\_lm.pt}。在手动测试题「今有田廣十五步，從十六步」上，模型生成「一畝」，与原书答案一致，说明模型记住并能复现部分典型题型。
...
井徑五尺，不知其深 & 五丈七尺五寸 & 五丈三尺三寸、少半寸 \\
股四尺，弦五尺，求句 & 三尺 & 八尺六寸、太半寸 \\
\bottomrule
\end{tabular}
\end{table}

从样例看，模型已经学习到《九章算术》的答案格式、数字表达和单位体系，例如「畝」「尺」「寸」「分之」等模式会稳定出现；但它并没有可靠掌握精确算术推理。关键词命中率达到 1.0，更多反映模型能生成题型相关数字和单位，不代表每道题都计算正确。
